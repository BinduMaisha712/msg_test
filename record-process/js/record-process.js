  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (data.oostatus == "outOfStock") {
        console.log('function: checkStock() | line no -> 42');
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status) {
        ...
        if (data.result == 'Enter OTP') {
          $(".log-in-section").attr("style", "display:none");
          $(".otpVerify").attr("style", "display:block");
          timer(60);        
          var userInfo = $("#emailId").val();
          $('#userInfo').val(userInfo);
        } if ((data.type == 'Login'|| data.type == 'Register') && (data.result == 'Agent LogIn Successfully' ||data.result == 'Agent Registered Successfully. Please wait for account verification from admin.')) {
...
$("#agentLoginForm3").on("submit", function (e) {
  e.preventDefault();
  var frm = $("#agentLoginForm");
  var form = $("#agentLoginForm")[0];
  var data = new FormData(form);
  console.log("fas djf");
  $.ajax({
    url: "ajax/agent.php",
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status == "Not LogIn") {
        ...
        url = "'" + url + "'";
        var html =
          '<span class="ans-wish"><a onclick="removeFromWishList(' +
          productId +
          ",this.id," +
          url +
          ')" id="' +
          divId +
          '"><i class="fa fa-heart" aria-hidden="true" style="background-color: white;padding: 8px;color: red;border-radius: 17px;               border: 1px solid #e0e0e0;font-size: 16px;"></i></a></span>';
        // console.log(html);
        if (currentPage == "index.php" || currentPage == "") {
          ...
          var html1 =
            '<li><a onclick="removeFromWishList(' +
            productId +
            ",this.id," +
            url +
            ')" id="' +
            divId +
            '" title="Already Added" ><i class="fa fa-heart" aria-hidden="true" style="color: red;font-size: 18px;"></i> Already Added</a></li>';
          console.log(html1);
          $(".addWish").html(html1);
          $("." + divId).html(html);
        } else {
  ...
  confirmDialog("Are You sure! You want to remove?", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        url = "'" + url + "'";
        var html =
          '<span class="ans-wish"><a onclick="addToWishList(' +
          productId +
          ",this.id," +
          url +
          ')" id="' +
          divId +
          '" href="javascript:;"><i class="fa-regular fa-heart"></i></a></span>';
        if (currentPage == "index.php" || currentPage == "") {
          $("#newWish_" + productId).html(html);
          $("#topWish_" + productId).html(html);
          $("#bestWish_" + productId).html(html);
          ...
          var html1 =
            '<li><a onclick="addToWishList(' +
            productId +
            ",this.id," +
            url +
            ')" id="' +
            divId +
            '" title="Add Wish List" class="wishlist">Add to Wish List</a></a></li>';
          $(".addWish").html(html1);
          $("." + divId).html(html);
        } else {
          $("." + divId).html(html);
...
function addToCart(formId, classType, btnType = "") {
  console.log("adding...");
...
function setModelProductId(id, div, type, classType) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.dis == 0) {
        $("#modelPrice" + div).html(
          '<span class="price-regular">' + currency + data.price + "</span>"
        );
      } else {
        $("#modelPrice" + div).html(
      ...
      if (!data.isCart) {
        ...
        html +=
          '<button class="ed-addToCart btn btn-cart"  type="submit" data-toggle="tooltip" title="Add To Cart" onclick="addToCart(' +
          formId1 +
          "," +
          className +
          ')">Add To Cart</button>';
        ...
        html +=
          '<button class="ed-addToCart btn btn-cart"  data-toggle="tooltip" title="Already Added" style="margin-right:61px;">Already Added</button><span class="sicker"><i class="fa fa-trash" onclick="removeFromCart(' +
          id +
          "," +
          divId +
          "," +
  ...
  confirmDialog("Are You sure! You want to remove?", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        var html =
          '<button class="ed-addToCart btn btn-cart"  data-toggle="tooltip" title="Add To Cart"  onclick="addToCart(' +
          formId1 +
          "," +
          className +
          ');">Add To Cart</button>';
        ...
        if (window.location.href.includes("product-detail.php")) {
          var proHtml =
            `<button class="quantity-button btn btn-cart2" type="submit" id="formId` +
            data.productId +
...
//search
// ================ code to fix autocomplete in mobile view
...
$(".productSearch2").on("keydown", "#search2", function (e) {
...
function hidesearchdiv2() {
  var x = document.getElementById("autocomplete2");
...
function orderBy(val, price = null) {
    let catType = $('#category_type').val();
    
    // console.log(categoryType+' --> getVals');
    let listingId = $('input[name="listingId"]').val();
  ...
  $.ajax({
    url: actionUrl,
    type: "POST",
    data: { orderByVal: val, pageNo: pageNo,filter_by: price , catType: catType,
              listingId: listingId,},
    // dataType: 'json',
    // added to show sweetAlert
    beforeSend: function () {
      Swal.fire({
        title: "<span>Please Wait... <br> <div class='SWLoader'></div></span>",
        html: "Request under processing, please do not lock the screen or leave the page.",
    ...
    success: function (data) {
      ...
      $('#filterProductId').html(data.html);
    //   $(".changeFilter").load(url + " #filterProductId");
      $(".shop-top-left").load(url + " #listing-stats");
...
function getVals() {
    let minPrice = Number($('.input-min').val());
    let maxPrice = Number($('.input-max').val());
    ...
    var pageNo = 1;  
    var actionUrl = "ajax/load_listing_products.php";
    var filter = '';  
    ...
    $.ajax({
        ...
        success: function (response) {
                if (response.trim() !== '') {
                    ...
                    $('#filterProductId').html(response); 
                    
                    var product_count = $('div.sp-product-item').length;
                    $("#totalProducts").html(product_count);
                    //$("#recordFrom").html(product_count);
                    $("#recordTo").html(product_count);
...
function checkboxFilter(element, type, classtypeId) {
  ...
  if(type == 'brand'){
      let catType = element.getAttribute('catType');
      let listingId = element.getAttribute('listingId');
      
        $.ajax({
            url: "ajax/load-products-custom.php",
            type: "GET",
            data: {
              action: action,
              type: type,
      ...
      console.log(catType, listingId);
      return;
  }
  // Weight Filter
  if(type == 'class0' || type == 'class1' || type == 'class2'){
      let catType = element.getAttribute('catType');
      let listingId = element.getAttribute('listingId');
      
        $.ajax({
            url: "ajax/load-products-weight.php",
            type: "GET",
            data: {
              action: action,
              type: type,
  ...
  $.ajax({
    ...
    //   location.reload();
      ...
      $("#recordFrom").html(data.recordFrom);
      $("#recordTo").html(data.recordTo);
      //var url = "listing.php?id=cat_id@"+classtypeId; //please insert the url of the your current page here, we are assuming the url is 'index.php'
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var proHtml =
        ...
        data.productId +
        `">
                    <a href="cart.php" class="quantity-button btn btn-cart2" id="formId` +
        data.productId +
        `Button"><span>Go To Cart</span></a>
                     <button type="button" class="quantity-button btn btn-cart2" onclick="removeFromCart(` +
        data.productId +
...
$(".phone").on("keyup", function (e) {
  ...
  var isValid = regexPattern.test(phno);

  if (!isValid || phno.length !== 10) {
    $("#" + this.id + "ErrMsg").html(
    ...
    $("#" + this.id + "ErrMsg").empty(); // Clear the error message
    $(".userRegBtn").show();
  }
});
  ...
  if (value != "") {
    ...
    if (!isValid) {
      ...
      $("#" + this.id + "ErrMsg").html(
        "<div class='err_msg' id='" +
          this.id +
          "ErrMsg'>Password must be between 8 to 15 characters and contain at least one numeric digit, one special character, one uppercase, and one lowercase letter</div>"
      );
    } else {
...
function removeCoupan(value) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var totalPriceVar =
        parseFloat(data.totalPrice) + 100 + parseFloat(data.gst);
      var roundedTotal = totalPriceVar.toFixed(2);
      $("#totalAmount").html(currency + roundedTotal);
      window.location.reload();
    },
    complete: function () {
      $("body").css("pointer-events", "auto");
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.status == "success") {
        $("#ErrMsg").attr(
          "style",
          "border: 1px solid #eed177!important;background: #eee3c0!important;padding: 10px;"
        );
        $("#ErrMsg").html(
          "Coupon Applied <b>" +
            $("input[name=coupanCode]").val() +
            '</b>   <span style="text-align: right;font-weight: 500;cursor: pointer; color: #ffffff;   background-color: #ed3237;font-size: 13px;padding: 4px 10px;border-radius: 12px;" onclick="removeCoupan(' +
            data.savePrice +
            ')">Remove</a>'
        );
        ...
        $("#coupanAppliedDiv").html(
          "<h4>Coupon Applied <b>(Save)</b></h4> <h4 id='cprice' class='price'>- " +
            currency +
            number_format_js(data.savePrice, 2) +
            "</h4>"
        );
        $("#couponAmount").val(number_format_js(data.gst, 2));
        var CheckoutTaxSection = $("#CheckoutTaxSection").text(
          number_format_js(data.gst, 2)
        );
...
function clickPaybutton() {
  ...
  if ($("#buyAsGuest").val() == "true") {
    ...
    if ($("input[name=newDifferentShippingFirstName]").val() == "") {
      $("input[name=newDifferentShippingFirstName]").val(
        $("input[name=newBillingAddressFirstName]").val()
      );
    }
    if ($("input[name=newDifferentShippingLastName]").val() == "") {
      $("input[name=newDifferentShippingLastName]").val(
        $("input[name=newBillingAddressLastName]").val()
      );
    }
    if ($("input[name=newDifferentShippingPhone]").val() == "") {
      $("input[name=newDifferentShippingPhone]").val(
        $("input[name=newBillingAddressPhone]").val()
      );
    }
    if ($("input[name=newDifferentShippingEmail]").val() == "") {
      $("input[name=newDifferentShippingEmail]").val(
        $("input[name=newBillingAddressEmail]").val()
      );
    }
  }
...
function checkStock() {
  $.ajax({
    ...
    success: function (res) {
      ...
      //   // console.log(data.oostatus)
      if (data.oostatus == "outOfStock") {
        console.log('function: checkStock()| line -> 1820');
        // display alert/notification
        $("#snackbarDefault").html('Selected Item is currently Out Of Stock...!');
        var x = document.getElementById("snackbarDefault");
        x.className = "show";
        setTimeout(function () {
          x.className = x.className.replace("show", "");
...
$("#btn54").on("click", function () {
  ...
  var isCashOnDelivery =
    $('input[name="paymentmethod"]:checked').val() === "cashondelivery";

  if (isCouponApplied && isCashOnDelivery) {
    // Display SweetAlert for invalid coupon on Cash On Delivery
    Swal.fire({
      icon: "error",
      title: "Invalid Coupon with Cash On Delivery",
      text: "The coupon code is not valid when choosing Cash On Delivery option.",
    });
  } else {
    // Continue with the default action (e.g., submitting the form)
...
$(document).on("click", "#testbtn", function (e) {
  ...
  formData = $(
    "#placeOrder,#addressDetailsForm,#coupanForm,#pymentMethodForm, #shippingDetails"
  ).serialize();
  console.log(formData);
  // return false;
  $.ajax({
          ...
          if (data.status == "cod") {
            ...
            var options = {
             ...
             // key: "rzp_live_9Tz7y5gTkyJGfJ",
              ...
              image:
                "https://micodetest.com/msg_updated/asset/image/logo/logo.png",
            ...
            rzp1.open();
          } else if (data.status == "wlt") {
            var orderId = data.orderId;
            $.ajax({
              type: "POST",
              url: "ajax/update_wallet_order.php",
              data: { orderId: orderId, action: "walletr" },
                ...
                if (data.status == true) {
                    $.ajax({
                      ...
                      success: function (data) {
                        ...
                        // Check the status returned from the PHP script
                        if (data.status == true) {
                          // Handle success
                  ...
                  console.log("Something Went Wrong In Order Process...");
                }
              },
            });
          }
...
$("#cancelOrder").on("submit", function (e) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      confirmDialog(
        "Are You sure! You want to cancel order?" + html,
        function () {
