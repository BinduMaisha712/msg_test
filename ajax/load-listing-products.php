...
if($filter==''){
	...
	{  
	   if($category_type == 'p.subcat_id'){
	       $sql = "SELECT * from products WHERE status='Active' AND subcat_id = $catidd group by group_code ORDER BY CAST(price AS DECIMAL) ".$_SESSION['filter']['orderBy']." LIMIT $offset, $limit";
	   }
	   else{
	       $sql = "SELECT * from products WHERE status='Active' AND cat_id = $catidd group by group_code ORDER BY CAST(price AS DECIMAL) ".$_SESSION['filter']['orderBy']." LIMIT $offset, $limit";
	   }
	}else{
	    if($category_type == 'p.subcat_id'){
	        $sql = "SELECT * from products WHERE status='Active' AND subcat_id = $catidd group by group_code order by id desc LIMIT $offset, $limit";
	    }
	    else{
	        $sql = "SELECT * from products WHERE status='Active' AND cat_id = $catidd group by group_code order by id desc LIMIT $offset, $limit";
	    }
	}
