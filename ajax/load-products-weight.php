...
// echo "$catidd | $condition | $category_type | $action";
if($action=='addFilter'){
    // Add the condition to the session
    if (!isset($_SESSION['weight'])) {
        $_SESSION['weight'] = array();
    }
    if (!in_array($condition, $_SESSION['weight'])) {
        $_SESSION['weight'][] = $condition;
    }
    $condition = '';
    $count = 0;
    foreach($_SESSION['weight'] as $key => $val){
        if($count){
            $condition .= ' OR '.$val;
        }
        else{
    ...
    if (($key = array_search($condition, $_SESSION['weight'])) !== false) {
        unset($_SESSION['weight'][$key]);
    }
