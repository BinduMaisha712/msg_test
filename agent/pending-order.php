...
?>
        ...
        <main class="main--container">
            ...
            <section class="main--content">
                <div class="panel">
                    <!-- Records List Start -->
                    <div class="records--list" data-title="Pending Order">
                        <table id="recordsListView">
                            ...
                            <?php 
                                ...
                                if(!empty($_GET['fromdate']) && !empty($_GET['todate'])){
                                    ...
                                    $todate=$_GET['todate'];
                                    // $query=mysqli_query($conn,"SELECT * FROM `order_details` WHERE tracking_id NOT IN (SELECT tracking_id FROM `order_status`) AND `date` BETWEEN '$fromdate' AND '$todate' ORDER BY id DESC");
                                    $mysqlQuery = "SELECT * FROM `order_tbl`  WHERE `date` BETWEEN '$fromdate' AND '$todate' order_status = 'Pending' and userid='$agentuid' ORDER BY `id` DESC";
                                    $query=mysqli_query($conn, $mysqlQuery);
                               }else{
                                $query=mysqli_query($conn,"SELECT * FROM `order_details` WHERE tracking_id NOT IN (SELECT tracking_id FROM `order_status`) ORDER BY id DESC");
                                $mysqlQuery = "SELECT * FROM `order_tbl`  WHERE order_status = 'Pending' AND userid='$agentuid' ORDER BY `id` DESC";
                                ...
                                <tbody id="collapse<?php echo $sr; ?>" class="collapse <?php if($sr==1) echo "show"; ?>" aria-labelledby="heading<?php echo $sr; ?>" data-parent="#accordionExample">
                                    ...
                                    <?php
                                        $pro_select=mysqli_query($conn,"SELECT * FROM `order_details` WHERE tracking_id NOT IN (SELECT tracking_id FROM `order_status`) AND order_id='$order_id'");
                                        $sn=1;
