        ...
        <section class="main--content">
            <div class="panel">
                <div class="records--body">
                    <div class="title">
                        <h6 class="h6">Terms And Conditions Details</h6>
                    </div>
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade show active" id="tab01">
                            <form action="" method="post" enctype="multipart/form-data" name="form">  
                                <?php
                                $pquery=mysqli_query($conn,"SELECT * FROM `terms&conditions_title` WHERE id=$tid"); //products select query
                                $pdata=mysqli_fetch_array($pquery);

                                $des_query=mysqli_query($conn,"SELECT * FROM `terms&condition` WHERE title_id=$tid"); //description select query ?>
...
if(isset($_POST['submit']))
{
    ...
    $up=mysqli_query($conn,"UPDATE `terms&conditions_title` SET `title`='$title' WHERE id=$tid");
    ...
    foreach($description as $key => $value)
    {
       ...
       //echo $key."- ".$value." id- ".$rid[$key]."<br/>";
        $auid=$rid[$key];
        $update=("UPDATE `terms&condition` SET `description` = '$value' WHERE title_id=$tid AND id = '$auid'") ;
        $des_query=mysqli_query($conn,$update) or die(mysqli_error());
    }
    if(isset($_POST['add_description']))
    {
        ...
        {
            $value = mysqli_real_escape_string($conn, $_POST['add_description']);
           $query=mysqli_query($conn,"INSERT INTO `terms&condition`(`title_id`, `description`) VALUES ('$tid','$value')");
        }
    }
    if($des_query)
    {  
        ...
        echo"var delay = 1000;setTimeout(function(){ window.location = 'terms-n-conditions-view.php'; }, delay);";
        echo "</script>";
    }
    else
    {
