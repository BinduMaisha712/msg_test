...
<main class="main--container">
      ...
      <div class="records--body">
        ...
        <div class="info">
          <p><strong>Info!</strong> You can create product based on classtype. you can enter price
            individually by product classtype. you can add or remove deal based on product classtype; if you
            cannot get any deal on product then unchecked add deal and if you want to meta then select now
          ...
          <div class="tab-pane fade show active" id="tab01">
            <div class="panel-content">
              <form action="" method="post" enctype="multipart/form-data" name="form">
                    ...
                    <select name="category" id="category" class="form-control" required>
                      ...
                      ?>
                        <option value="<?php echo $data['id']; ?>"
                          data-issubcat="<?= $data['issubcategory'] ?>"
                          <?php if ($data['issubcategory'] == 'No') { ?>
                          data-clastype='<?= $data['classtype_id'] ?>' <?php
                ...
                <!--    </div>-->
                <!--</div>-->
                <div id="todayDealsApplicableList" style="display:none">
                </div>
                <!--                                                                  -->
                ...
                <div class="form-group row">
                  <span class="label-text col-md-3 col-form-label">Specifications: </span>
                  <!-- <div class="col-md-9"> -->
                  <div class="field_wrapper form-group">
                    <div class="input-group">
                      <div class="row">
  ...
  <script type="text/javascript">
            ...
            if (issubcategory == 'No') {
              ...
              $.ajax({
                ...
                success: function(response) {
                  ...
                  if (fclastype == 16) {
                    var content = '<div class="row" id="div1_9"><div class="col-md-12"><div class="card card-outline-danger p-2"><div class="form-group row"><span class="label-text col-md-3 col-form-label">Start Date: </span><div class="col-md-9"><input type="date" name="sdate[0][]" class="form-control"  ></div></div><div class="form-group row"><span class="label-text col-md-3 col-form-label">Start Time: </span><div class="col-md-9"><input type="time" name="stime[0][]" class="form-control"  ></div></div><div class="form-group row"><span class="label-text col-md-3 col-form-label">End Date: </span><div class="col-md-9"><input type="date" name="edate[0][]" class="form-control"  ></div></div><div class="form-group row"><span class="label-text col-md-3 col-form-label">End Time: </span><div class="col-md-9"><input type="time" name="etime[0][]" class="form-control"  ></div></div><hr/><div class="form-group row"><span class="label-text col-md-3 col-form-label"> Deal Stock : </span><div class="col-md-9"><input type="text" name="dealstock[0][]" class="form-control productPriceList"  placeholder="No. of Deal Item in Stock" ></div></div><div class="form-group row"><span class="label-text col-md-3 col-form-label">Deal Price: </span><div class="col-md-9"><input type="text" name="deal_price[0][]"  onkeypress="if ( isNaN(this.value + String.fromCharCode(event.keyCode) )) return false;"  id="dealamount0" onblur="check_dealamount(0)" class="form-control" placeholder="Only Enter Rs" ><span class="help-block" id="dealam0"></span></div></div></div></div>';
                    $('#todayDealsApplicableList').html(content);
                  }
    ...
    function getSize(sel, random) {
      ...
      if ($("#" + selId).is(':checked')) {
        ...
        // opts.push(opt);
        //console.log(opt.value);
        var jso = $(('#sixeids_' + random)).val();
        console.log(jso);
        if (jso != '') {
          jso = jso + ','
        }
        jso = jso + sizeSymbole[0];
        console.log(jso);
        $(('#sixeids_' + random)).val(jso);
        var i = Math.floor(Math.random() * 1000) + 1;
        ...
        var content = '<hr/><div class="form-group row"><span class="label-text col-md-3 col-form-label">' +
          ...
          ' id="disam_' + id1 + '"></span></div>' +
          '<span class="label-text col-md-3 col-form-label">Agent Commission  (%): </span>' +
          '<div class="col-md-9"><input type="number" name="user_disc[' + random + '][]" class="form-control" placeholder="Only Enter Rs" max="100"></div></div>'
        ...
        var json = $('#sixeids_' + random).val().split(',');
        for (var i = 0; i < json.length; i++) {

          if (json[i] === sizeSymbole[0]) {
            json.splice(i, 1);
            i--;
          }
        }
        $('#sixeids_' + random).val(json);
      }
    ...
    if (mysqli_num_rows($sel_query) > 0) {
      ...
      if (count($classtype) == 2) {
        foreach ($_POST['productColor'] as $color) {
          ...
          $abc = array();
          //   $key = array_search($color, $_POST['productColor']);
          $sixeids = explode(',', $_POST['sixeids'][$key]);
          foreach ($sixeids as $size) {
            $k = findKeysBySubstring($_POST['productSizes'][$key], $size . "@");
            $abc[] = $_POST['productSizes'][$key][$k];
          }
          $_POST['productSizes'][$key] = $abc;
          ...
          foreach ($_POST['productSizes'][$key] as $size) {
            ...
            $user_disc = $_POST['user_disc'][$key][$i];
            ...
            $mySqlQuery = "INSERT INTO `products`(`cat_id`, `subcat_id`, `product_name`,`brand`,`class1`, `price`,`discount`,`userdisc`, `length`,`height`,`width`, `weight`,`hot_deals`,`new_arrivals`,`top`,`cod`,`stock`,`in_stock`,`minimum`,`maximum`,`class0`,`product_code`,`group_code`,`date`,`time`, `tax`) VALUES "
              . "('$category','$subcategory','$product','" . addslashes($_POST['brand']) . "','" . $color . "','" . $price . "','" . $disc_price . "','$user_disc','$length','$height','$width','$weigth', '$hot','$new','$top','$cod','$stock','$inStockQ','$min','$max','" . $sizeInfo[0] . "','" . $p_uniqu . "','" . $groupCode . "','$date','$time', '$product_tax')";
            ...
            if (mysqli_num_rows($sel_query) > 0) {
              ...
              // exit();
              $dquery = mysqli_query($conn, "INSERT INTO `specification`(`cat_id`, `subcat_id`, `p_id`, `specification`) VALUES ('$category','$subcategory','$lastProductID','$specification')") or die(mysqli_error());
          ...
          foreach ($image_name as $imageKey => $value) {
            ...
            $temp = explode(".", $value);
            $newfilename = round(microtime(true)) . $ik . $key . '.' . end($temp);
        ...
        $color = "";
        $mySqlQuery = "INSERT INTO `products`(`cat_id`, `subcat_id`, `product_name`,`brand`,`class1`, `price`,`discount`,`userdisc`,`length`,`height`,`width`, `weight`,`hot_deals`,`new_arrivals`,`top`,`cod`,`stock`,`in_stock`,`minimum`,`maximum`,`class0`,`product_code`,`group_code`,`date`,`time`,`tax`) VALUES "
          . "('$category','$subcategory','$product','" . addslashes($_POST['brand']) . "','','" . $price . "','" . $disc_price . "','$user_disc','$length','$height','$width','$weigth','$hot','$new','$top','$cod','$stock','$inStockQ','$min','$max','','" . $p_uniqu . "','" . $groupCode . "','$date','$time', '$product_tax')";
        $query = mysqli_query($conn, $mySqlQuery) or die(mysqli_error());
        ...
        foreach ($_POST['productColor'] as $color) {
          ...
          foreach ($_POST['productSizes'][$key] as $v3 => $size) {
            ...
            foreach ($_POST['productSizesClass'][$key] as $k1 => $class) {
              ...
              $user_disc = $_POST['user_disc'][$key][$v3][$k1];
              ...
              $mySqlQuery = "INSERT INTO `products`(`cat_id`, `subcat_id`, `product_name`,`brand`,`class1`, `price`,`discount`,`userdisc`,`length`,`height`,`width`, `weight`,`hot_deals`,`new_arrivals`,`top`,`cod`,`stock`,`in_stock`,`minimum`,`maximum`,`class0`,`class2`,`product_code`,`group_code`,`date`,`time`, `tax`) VALUES "
                . "('$category','$subcategory','$product','" . addslashes($_POST['brand']) . "','" . $color . "','" . $price . "','" . $disc_price . "','$user_disc','$length','$height','$width','$weigth','$hot','$new','$top','$cod','$stock','$inStockQ','$min','$max','" . $sizeInfo[0] . "','" . $classInfo[0] . "','" . $p_uniqu . "','" . $groupCode . "','$date','$time', '$product_tax')";
              // echo $mySqlQuery;
              $query = mysqli_query($conn, $mySqlQuery) or die(mysqli_error());
  ...
  function findKeysBySubstring($array, $substring)
  {
    $keys = [];
  ...
  //       imagegif($image, $destination); // Removed $source argument
  //     } else {
