                            ...
                            <?php 
                                ...
                                if(!empty($_GET['fromdate']) && !empty($_GET['todate'])){
                                    ...
                                    if($_GET['status']=='Cancelled'){
                                        ...
                                        $query=mysqli_query($conn,"SELECT * FROM `order_details`  WHERE tracking_id NOT IN (SELECT tracking_id FROM `delivery_schedule`) AND tracking_id NOT IN (SELECT tracking_id FROM `order_status` WHERE tracking_status='Cancelled') AND `date` BETWEEN '$fromdate' AND '$todate'  ORDER BY `id` DESC");
                                     }
...
<?php
    if($_GET['status']=='Cancelled'){
        $pro_select=mysqli_query($conn,"SELECT * FROM `delivery_schedule` WHERE order_id='$order_id' AND delivery_status='Cancelled'  AND delivery_status_by='deliverymen' AND `dmen_date` BETWEEN '$fromdate' AND '$todate'");
    }else{
        $pro_select=mysqli_query($conn,"SELECT * FROM `order_details`  WHERE order_id='$order_id' AND `date` BETWEEN '$fromdate' AND '$todate' AND tracking_id NOT IN (SELECT tracking_id FROM `delivery_schedule`)");
}
    $sn=1;
